 ...
 * 
 * Architecture:
 *   - 10 column-groups (one PLIO each, cols 0-4, 5-9, ..., 45-49)
 *   - 8 PSPs per group (one per row, pktsplit<8>/pktmerge<8>)
...
int run(int argc, char* argv[]) {
    ...
    if (!quiet_mode) {
        ...
        std::cout << " PSPs/group:    " << psps_to_process << " (max: " << PSPS_PER_TRUNK << ")" << std::endl;
    ...
    std::cout << "   Data load:        " << metrics.data_load_time_ms << " ms" << std::endl;
