...
double rn(int idummy){
  ...
  if (!rand_initialized){
    ...
    global_rand.rmarin(1802, 9373);
  }

  while(true){
    ran = global_rand.ranmar();
    if (ran>1e-16) break;
  }
  return ran;
}

// Function to set RAMBO seed externally for reproducible testing
void set_rambo_seed(int ij, int kl){
  rand_initialized = true;
  global_rand.rmarin(ij, kl);
}
